...
| `stfw scenario doc <name> [--out]` | シナリオを Markdown ドキュメントへ投影 (tree → doc) |
| `stfw scenario spec <name> [--out]` | シナリオを spec yaml へ export (tree → spec、往復の出口) |
| `stfw scenario scaffold <spec.yml> [--force] [--prune]` | spec からシナリオ骨格を生成 (spec → tree、往復の入口)。`--prune` で spec との差分同期 |
