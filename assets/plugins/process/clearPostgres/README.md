...
PostgreSQL のテーブルの全行を DELETE する Arrange フェーズ (初期化) の組込みプラグイン。
...
`tables` の全テーブルを `DELETE FROM` で空にする。全削除は 1 トランザクションで実行され、
途中で失敗した場合は何も削除されない (all-or-nothing)。データ投入 (importPostgres) の
先行プロセスとして使う。

- **FK 制約は無効化しない**。行単位で FK 検証されるため、`tables` は **FK の子 → 親の順**に列挙する
- 参照行が残る親を削除しようとすると PostgreSQL の FK 違反エラーが stderr に出て失敗する
- `ON DELETE CASCADE` が宣言されているテーブルへは DDL どおり波及する
- シーケンス (IDENTITY) はリセットしない。完全リセットが必要ならカスタムプラグインで行う
