...
func TestRedisEncodeRow(t *testing.T) {
	cases := []struct {
		...
		{"RedisEncodeRow_string型の場合_生値をvalue列に出力すること", "k", "string", "-1", `"hello"`, "k,string,-1,hello\n"},
		// list は順序保持の JSON 配列。CSV クオートされる。
		{"RedisEncodeRow_list型の場合_順序保持のJSON配列を出力すること", "k", "list", "-1", `["a","b"]`, "k,list,-1,\"[\"\"a\"\",\"\"b\"\"]\"\n"},
		// set は要素ソート後の JSON 配列。
		{"RedisEncodeRow_set型の場合_要素ソート後のJSON配列を出力すること", "k", "set", "-1", `["y","x"]`, "k,set,-1,\"[\"\"x\"\",\"\"y\"\"]\"\n"},
		// hash は field ソート後の JSON オブジェクト (json.Marshal がキー昇順)。
		{"RedisEncodeRow_hash型の場合_fieldソート後のJSONオブジェクトを出力すること", "k", "hash", "10", `["f2","v2","f1","v1"]`, "k,hash,10,\"{\"\"f1\"\":\"\"v1\"\",\"\"f2\"\":\"\"v2\"\"}\"\n"},
		// zset は member ソート後の [[m,s],...]。
		{"RedisEncodeRow_zset型の場合_memberソート後の配列を出力すること", "k", "zset", "-1", `["m2","2","m1","1"]`, "k,zset,-1,\"[[\"\"m1\"\",\"\"1\"\"],[\"\"m2\"\",\"\"2\"\"]]\"\n"},
	}
	for _, c := range cases {
...
func TestRedisEncodeRowStringSpecial(t *testing.T) {
	t.Run("RedisEncodeRow_値にカンマ・改行を含む場合_CSVクオートで壊れないこと", func(t *testing.T) {
		// Act
...
func TestRedisDecode(t *testing.T) {
	t.Run("RedisDecode_全型のCSVの場合_redis-cliコマンドに復元されること", func(t *testing.T) {
		// Arrange
		...
		// Act
		err := RedisDecode(strings.NewReader(csvIn), &out)
...
func TestRedisQuoteBinary(t *testing.T) {
	t.Run("redisQuote_非表示文字と特殊文字を含む場合_エスケープされること", func(t *testing.T) {
		// Act
		// 非表示文字は \xNN、" と \ はエスケープ。
...
func TestTTLSeconds(t *testing.T) {
	t.Run("ttlSeconds_各種TTL文字列の場合_秒数に変換されること", func(t *testing.T) {
		// Arrange
