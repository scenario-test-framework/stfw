...
func TestLoadInventory(t *testing.T) {
	t.Run("LoadInventory_複数グループが定義される場合_グループごとにホストが解決されること", func(t *testing.T) {
		// Arrange
		projDir := t.TempDir()
...
func TestLoadInventoryMergesDuplicateGroups(t *testing.T) {
	t.Run("LoadInventory_同名グループが重複する場合_ホストがマージされること", func(t *testing.T) {
		// Arrange
		projDir := t.TempDir()
		writeInventory(t, projDir, "staging.yml", `stfw_inventory:
  - web:
...
func TestLoadInventoryFileNotFound(t *testing.T) {
	t.Run("LoadInventory_ファイルが存在しない場合_エラーであること", func(t *testing.T) {
		// Arrange
		projDir := t.TempDir()

		// Act
		_, err := LoadInventory(projDir, "missing.yml")
...
func TestLoadInventoryStructuredEntries(t *testing.T) {
	t.Run("LoadInventory_文字列と構造化エントリが混在する場合_ホスト名を解決すること", func(t *testing.T) {
		// Arrange
		projDir := t.TempDir()
		writeInventory(t, projDir, "staging.yml", `stfw_inventory:
  - web:
...
`)
	...
	t.Run("LoadInventoryHostArch_構造化エントリが存在する場合_ホストごとのarchを返すこと", func(t *testing.T) {
		// Arrange
		projDir := t.TempDir()
		writeInventory(t, projDir, "staging.yml", `stfw_inventory:
  - web:
...
func TestLoadInventoryHostArchConflict(t *testing.T) {
	t.Run("LoadInventoryHostArch_同一ホストにarch競合がある場合_エラーであること", func(t *testing.T) {
		// Arrange
		projDir := t.TempDir()
		writeInventory(t, projDir, "staging.yml", `stfw_inventory:
  - a:
...
`)
		...
		// Act
		_, err := LoadInventoryHostArch(projDir, "staging.yml")
...
func TestLoadInventoryHostArchSameArchAcrossGroups(t *testing.T) {
	t.Run("LoadInventoryHostArch_複数グループで同一archの場合_許容されること", func(t *testing.T) {
		// Arrange
		projDir := t.TempDir()
		writeInventory(t, projDir, "staging.yml", `stfw_inventory:
  - a:
