...
|----|----|-----------|---------|-----------|:----------:|------|
| AG-001 | BC-002 | E-015 | E-016, E-020 | • run 実行は対象シナリオが存在し run 前静的検証を通過していること<br/>• Error 発生（終了コード 0・3 以外、または compare の on_mismatch=error での比較不一致）後の後続ステップは実行せず Blocked として記録し停止する。終了コード 3 は Warn として記録し続行する<br/>• run_id は _{YYYYMMDDHHMMSS}_{PID} 形式で一意に採番する<br/>• 終了状態（Success / Warn / Error）からの再遷移は禁止。再実行は新しい run_id の別実行として扱う<br/>• ジャーナルからの復元（リプレイ）経路でも生成時と同じ状態遷移検証を通す | ユーザー指定 | 承認済み v1.0 計画で確定した集約（集約 = Run、ジャーナル = Run のイベントストリーム）。メンバー詳細の最終確定は dist-spec or ddd-tactical-implementation で行う |
...
| SR-002 | 実行ログ運用 | 実行ログは stderr への構造化ログ（シークレットマスキング済み）として出力する（stdout はコマンド出力専用）。ファイルログ（.stfw/stfw.log への集約・日次ローテーション・terminal 実行時のカラー出力）は要求撤回により持たない。ログレベル（trace〜error、デフォルト info）は stfw.yml またはコマンドオプションで変更できる。ログの保管は呼び出し元（terminal / CI のログ保持）に委ね、実行結果の保管期間（1 ヶ月）は実行ジャーナル・HTML レポートのハウスキープ（stfw.housekeep.retention）で担保する | NFR C.6.1.1 ログ保管期間(Lv1) / C.6.1.2 ログ種別への対応。ユーザー指定: ファイルログ要求（DIST-002）はプロダクトオーナー決定で撤回され、stderr 構造化ログと HTML レポート・OTLP トレースの可視化で運用要求を満たす | 情報: 実行ログ, バリエーション: ログレベル, NFR C.6.1.1, NFR C.6.1.2 | ユーザー指定 |
...
| CTP-005 | 再実行は新 run_id の別実行 | 階層実行ステータスの終了状態（Success / Warn / Error）からの再遷移は行わず、再実行は常に新しい run_id の別実行として扱う。ジャーナルは追記専用でイベントの訂正・削除を行わない | 実行結果の不変性と監査可能性を保ち、冪等性の議論を「実行単位の分離」で単純化するため | 条件: run_id の採番・保持, 状態モデル「階層実行ステータス」 | ユーザー指定 |
...
| CTP-007 | 性能・拡張性は単一利用者・逐次実行前提 | 同時アクセス・スループットのスケールアウトはスコープ外とする（利用者ローカルで 1 実行 = 1 プロセス）。業務日付をまたぐ長時間バッチ実行は増分レポートと OTel トレースで進捗確認する。性能テストは実施しない。組込み parallel プロセスタイプによる子プロセス並走は単一実行内の実行時間短縮であり、複数利用者の同時アクセスやスループットのスケールアウトとは区別する（NFR B.x のグレードは引き上げない）。実行ホストの性能向上はスケールアップで対応する | NFR B.1.1.1 同時アクセス(Lv1) / B.1.1.3 / B.1.2.1 / B.2.1.2 スループット(Lv1) / B.2.2.1 バッチ処理時間(Lv1・ユーザー確定) / B.3.1.1 CPU 拡張性(Lv1) / B.4.1.1 性能テスト(Lv0) への対応。ユーザー指定: parallel は単一実行内の子プロセス並走でありスケールアウト要求（NFR B.x）の対象外とする位置づけを変更要望で確定 | 条件: parallel 子プロセスの並走実行, NFR B.1.1.1, NFR B.1.1.3, NFR B.1.2.1, NFR B.2.1.2, NFR B.2.2.1, NFR B.3.1.1, NFR B.4.1.1 | ユーザー指定 |
...
| CTR-003 | 構造化ログの stderr 出力 | CLI 本体のログは stderr への構造化ログとして出力する（stdout はコマンド出力専用）。単一ファイル（.stfw/stfw.log）への集約は要求撤回により行わない。OTLP 送信失敗等の警告も本ログに記録する | NFR C.6.1.2 ログ種別への対応。ユーザー指定: ファイルログ要求の撤回（障害調査は stderr 構造化ログ + HTML レポート / OTLP トレースで行う） | 情報: 実行ログ, NFR C.6.1.2 | ユーザー指定 |
...
| LR-004 | 不正な状態遷移は error 返却 | 状態モデル「階層実行ステータス」（Started → Success / Warn / Error）と状態モデル「ステップ実行ステータス」（Pending → Success / Warn / Error / Blocked）は別型として実装し、不正遷移は panic せず error を返す | 外部から編集可能なジャーナルのリプレイ経路でも安全に検証を通すため | 状態モデル「階層実行ステータス」, 状態モデル「ステップ実行ステータス」 | ユーザー指定 |
...
| CLP-003 | ロギング方針 | 構造化ログを stderr へ出力する（シークレットマスキング済み）。ファイルログ（.stfw/stfw.log・日次ローテーション・terminal 実行時カラー）は要求撤回により持たない。ドメイン層はログ出力しない。ログレベルは trace〜error でデフォルト info | NFR C.6.1.2 ログ種別への対応と、レイヤー別ログ責務の明確化（ファイルログ要求はプロダクトオーナー決定で撤回） | 情報: 実行ログ, バリエーション: ログレベル, NFR C.6.1.2 | ユーザー指定 |
...
| timezone | string | timezone（env 公開のみの任意キー。実装は業務日付判定・レポート時刻表記に参照しない。時刻はプロセスのローカルタイムゾーンが正） | Yes |  |
| process_parallel_max_parallel | integer | 組込み parallel プロセスの同時実行数上限の既定（stfw.process.parallel.max_parallel。同梱デフォルト 0 = 上限なし。プロジェクト config/plugins/process/parallel/config.yml・プロセス config/config.yml で上書き可・後勝ち） | Yes |  |
...
| children | text | 子プロセス（組込み parallel タイプ配下のみ。親と同形式 _{seq}_{group}_{type}。入れ子禁止・子 0 件は定義不正。子にも通常プロセスと同じ検証を適用） | Yes |  |
...
| E-011 | 1:N | 組込み parallel タイプのプロセスは配下に子プロセス（親と同形式）を持ち、子を並走実行する（子 1 件 = ジャーナル上の 1 ステップ） |
...
| 属性名 | 型 | 説明 | NULL | PK |
|--------|-----|------|:----:|:--:|
| log_stream | string | 出力先ストリーム（stderr への構造化ログ。stdout はコマンド出力専用。ファイルログは要求撤回により持たない） | No | Yes |
...
| 対象エンティティ | カーディナリティ | 説明 |
|-----------------|:---------------:|------|
| E-002 | N:1 | プロジェクト操作時の障害調査ログとして stderr へ出力される（ファイル保管しない） |
| E-015 | N:1 | run 実行中のログ（OTLP 送信失敗の警告を含む）として出力される |
...
| E-017 | キャッシュ | stderr ストリームへの出力のみ（ファイル永続化なし。保持は呼び出し元の terminal / CI ログに委ねる。実行結果の永続化はジャーナル E-020・HTML レポート E-021 が担う） | ユーザー指定 |
