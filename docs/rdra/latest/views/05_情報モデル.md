...
| プロジェクト環境管理 | プロジェクト設定（stfw.yml） | project_version、loglevel、inventory（参照ファイル名）、webhooks.urls、webhooks.on_start、webhooks.on_success、webhooks.on_error、server.bind、server.port、server.db_mode、server.max_task_threads、server.timezone |  | ログレベル、DB モード、webhook 通知設定 | ログレベル・inventory 参照・webhook・server 設定を保持し、デフォルト→プロジェクトの順に上書きして全スクリプトへ環境変数として公開するため |
...
| 実行管理 | 実行ログ | ログファイル（.stfw/stfw.log）、日次ローテーション、ログレベル（trace〜error）、シークレットマスキング済みログ行、セクション別処理時間 |  | ログレベル | 実行状況の追従表示・障害調査のためのログ（日次ローテーション・シークレットマスキング済み）を集約管理するため |
...
| 通知管理 | webhook payload | webhook_id（run_id + digdag task_name から導出）、parent_id（run > scenario > bizdate > process の親子ツリー）、type（run / scenario / bizdate / process）、status（Started / Success / Error）、create_time、start_time、end_time、processing_time、stfw.host / user / home / version、digdag.url / version、project.home / version、階層別属性（run: run_id / workspace_dir / params、scenario: name、bizdate: dirname / seq / bizdate、process: dirname / seq / group） | 階層実行ステータス | 階層タイプ（webhook type）、webhook イベント種別、webhook 通知設定 | run > scenario > bizdate > process の各階層の開始・終了（成功/失敗）を親子ツリー構造の通知データとして外部システムへ送るため。階層実行ステータスは payload の status 属性として永続化される |
| 通知管理 | ステップ実行結果 | script_name、result（Pending / Success / Error / Blocked）、start_time、end_time、processing_time | ステップ実行ステータス | 終了コード | スクリプト単位の実行結果（result・処理時間）を process の webhook payload に含め、失敗時の調査に使うため。result はスクリプトの終了コード（0/3/6）を基準にステップ実行ステータスとして確定する |
