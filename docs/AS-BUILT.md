...
| 項目 | 仕様 |
|---|---|
| バージョン | `stfw --version`。ビルド時に `-ldflags -X ...cli.Version=` で注入（未注入時 `1.1.0-dev`） |
...
> 根拠: `assets/plugins/process/compare/`, `test/acceptance/testdata/script/compare_expectとactualを突合する場合_合否を判定すること.txtar`, compare-files `internal/bulk/dir.go`（`relPathList`/`Counts.ProcessStatus`）・`internal/status/status.go`（`ExitCode`）
