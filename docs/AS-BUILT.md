...
5. end-of-test サマリを `${stfw_process_dir}/evidence/summary.json` へ出力し、そのサマリから自己完結の HTML レポート `${stfw_process_dir}/evidence/report.html` を生成する（k6 本体の web dashboard は負荷テスト向けで単発 Act では生成されないため、サマリからプラグイン側で常に作る。k6 失敗時もサマリが出るため report.html は残す）。
