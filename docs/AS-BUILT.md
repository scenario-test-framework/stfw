...
> 実装状況: 本節はプラグイン契約の**基盤**（P1）を記述する。収集系プラグイン本体（collectFile / collectLog）は §4.8（P2）、RDBMS 系（export / import / clear × MySQL / PostgreSQL）と secret 値の Masker 登録経路は §4.9（P3）、KVS 系（Redis）は §4.10（P4）、compare（Assert）は §4.11（P5）、invoke（Act: invokeWeb / invokeRest）は §4.12（P6）で実装済み。全組込みプラグインが実装完了。
