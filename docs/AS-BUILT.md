...
| 実行モデル | 逐次実行（1 実行 = 1 プロセス）。エラー時は後続の兄弟ノードを実行せず停止。例外として組込み `parallel` プロセス配下の子プロセスのみ並走する（§4.14） |
| 実行分離 | 実行はシナリオツリーを run ワークスペース（`.stfw/runs/{run_id}/workspace/{scenario_name}/`）へ複製した上で行い、`scenario/` 配下（正本）には書き込まない。複数の `stfw run`（同一シナリオを含む）を同一プロジェクトで並走できる（§5.7） |
...
| 項目 | 仕様 |
|---|---|
| バージョン | `stfw --version`。ビルド時に `-ldflags -X ...cli.Version=` で注入（未注入時 `1.5.0-dev`） |
...
| 3 | ExitWarn | `stfw run` が Warn あり・Error なしで完走した場合（差分確認モードの CI 検知用、§4.6）。`stfw plugin install` でインストール済みプラグインを再インストールした場合（v0.2 互換） |
| 6 | ExitError | 上記以外の全エラー（validate のエラー違反・run の Error・引数パースエラーを含む） |
...
| `stfw run [-d, --dry-run] [--from <path>] [--only <path>] [--resume[=<run_id>]] <scenario...>` | 1 つ以上のシナリオ名 | 内蔵ランナーで実行（§4, §5）。実行前に validate 相当の静的検証を自動実行。実行対象シナリオを run ワークスペースへ複製してから実行する（§5.7。`scenario/` 正本には書き込まない）。dry-run は execute / post_execute をスキップ。`--from` / `--only` は部分実行（§3.4。排他・指定時はシナリオ 1 つのみ。path は `{bizdate_dir}[/{process_dir}]` のシナリオ相対）。`--resume` は前 run のワークスペースを引き継ぐ resume（§5.8。値省略時は最新 run、指定時は `--resume=<run_id>` 形式）。終了コードは全 Success=0 / Warn あり（Error なし）=3 / Error あり=6（§4.6） |
| `stfw status [run_id]` | 省略時は最新 run | ジャーナルをリプレイして階層ツリーとステータスを表示。stdout が端末のときはステータスを色付けする（Success=緑 / Warn=黄 / Error=赤 / Blocked=灰。パイプ時は色なし） |
...
> 根拠: `internal/presentation/cli/*.go`（全コマンド定義）, `internal/domain/run/exitcode.go`, `internal/presentation/logger/logger.go`, `test/acceptance/testdata/script/{init_初期化する場合_雛形を展開し再初期化を拒むこと,new_シナリオを作成する場合_ディレクトリ規約で生成すること,validate_規約違反の場合_エラーになること,status_異常系の場合_エラーになること,report_run中とオンデマンドの場合_HTMLを生成すること,inventory_グループを問い合わせる場合_ホストを解決すること,secret_鍵生成とパスワード設定の場合_暗号化して保存すること,sshTrust_未登録ホストの場合_knownhostsへ登録すること,plugin_listとinstallの場合_解決順で操作できること,reverse_treeを入力する場合_specとdocを生成すること,scaffold_specを入力する場合_treeを生成すること,roundtrip_specを往復する場合_同一に戻ること}.txtar`
...
プロセスディレクトリは原則として階層の葉であり、配下のディレクトリは実行対象にならない。
唯一の例外は組込み `parallel` タイプで、プロセスディレクトリ直下に**子プロセスディレクトリ**
（親と同形式 `_{seq}_{group}_{type}`）を持ち、それらを並走させる（契約の詳細は §4.14）。
...
**部分実行（`stfw run --from` / `--only`）**

失敗した run のやり直し（途中から再開）と、特定ノードのピンポイント実行のための実行対象フィルタ。
実行順は変えず、実行対象のみを絞り込む。

| フラグ | 引数 | 動作 |
|---|---|---|
| `--from {bizdate_dir}[/{process_dir}]` | シナリオ相対パス（ディレクトリ名の完全一致） | 指定ノードより実行順で前のノードをスキップし、指定ノードから最後まで実行する。process まで指定した場合、その bizdate 内の先行 process のみスキップし、後続 bizdate は全 process を実行する |
| `--only {bizdate_dir}[/{process_dir}]` | 同上 | 指定ノードのサブツリーのみ実行する（bizdate 指定なら配下全 process、process 指定ならその process のみ） |

- `--from` と `--only` は**排他**（同時指定はエラー）。空値の明示指定（`--from=` / `--only=`）もエラー（全体実行へ化けさせない）。`--dry-run` とは併用できる。
- 部分実行時は run に指定できるシナリオは **1 つのみ**（複数指定はエラー）。
- 指定ノードが存在しない場合は **fail-fast**（run_id 採番・ジャーナル作成前にエラー。§3.5 の実行前ゲートと同じ扱い）。
- 静的検証（validate 相当・requires・接続情報直書き）は**フィルタ前のシナリオ全体**に対して行う。
- スキップされたノードは実行されず、**ジャーナルへ一切記録されない**（node_start / node_end もフックも無い）。
  実行されるノードの祖先階層のフック（run / scenario / 実行対象を含む bizdate の setup・teardown）は通常どおり実行される。
- **スキップした前提の再現はユーザー責務**: `--from` / `--only` で飛ばしたノードの副作用（データ投入等）は再現されない。
- **部分実行も新しい実行ワークスペースで実行される**（§5.7）: 過去 run のワークスペース内エビデンスは既定では参照されない。前 run の生成物（エビデンス・実行時生成の入力）を引き継いで途中から再開するには **`--resume` を併用する**（§5.8）。resume なしで compare を含む部分実行を行う場合は、依存する収集プロセスを実行対象に含める必要がある（例: `--only {bizdate_dir}` で bizdate 全体を実行する）。
...
> 根拠: `internal/domain/scenario/{bizdate,seq,group,name,dirname,hierarchy,tree,filter,violation}.go`, `internal/repository/scenariotree.go`, `internal/usecase/validate/validate.go`, `internal/usecase/runscenario/runscenario.go`（run 前検証・部分実行フィルタ）, `test/acceptance/testdata/script/{validate_規約違反の場合_エラーになること,run_fromを指定する場合_指定ノード以降のみ実行すること,run_onlyを指定する場合_指定サブツリーのみ実行すること,run_部分実行の指定が不正な場合_failfastすること}.txtar`
...
- 出力: リターンコード（0 = Success / **3 = Warn: 記録して続行** / その他の非 0 = Error: 停止 + Blocked 伝播。§4.6）
...
| フェーズ失敗 | 非 0・非 3 の時点で後続フェーズを実行せず Error。exit 3 は Warn として記録し後続フェーズを続行する（§4.6） |
| teardown 失敗 | プロセスの結果に影響しない（警告ログのみ） |
| teardown への追加 env | `stfw_run_status`（`Success` / `Warn` / `Error`）と `stfw_process_retcode`（`0` / `3` / `6`） |
...
- 同梱プラグインは実行前に run 単位の `.stfw/runs/{run_id}/plugins/process/{type}/` へ展開（毎回削除して再展開。shebang `#!` 始まりのファイルには 0755 を付与。§5.7）。`.stfw/plugins/process/{type}/` は `stfw plugin install` の展開先
...
4. **execute**（dry-run 時スキップ）: ステップを昇順に逐次実行。作業ディレクトリは `scripts/`。exit 0 → `Success`、exit 3 → `Warn`（記録して続行、§4.6）、その他の非 0 → `Error`（実行不能もエラー扱い・exit_code 6 で記録）。**Error 発生後の後続ステップは実行せず `Blocked` として記録**（Blocked 伝播。Warn は後続を止めない）
...
env 契約ゴールデンテスト（`test/acceptance/testdata/script/run_実行する場合_公開envが契約どおりであること.txtar` の `env_golden.txt`）で固定されている全変数:
...
| `stfw_scenario_dir` | シナリオディレクトリ絶対パス（scenario 階層以下で注入）。**run ワークスペース内の複製**（`.stfw/runs/{run_id}/workspace/{scenario_name}`）を指す（§5.7） |
| `stfw_scenario_name` | シナリオ名 |
| `stfw_bizdate_dir` | 業務日付ディレクトリ絶対パス（bizdate 階層以下で注入）。`stfw_scenario_dir` 配下（run ワークスペース内） |
...
| `stfw_process_dir` | プロセスディレクトリ絶対パス（process 階層で注入）。`stfw_bizdate_dir` 配下（run ワークスペース内） |
...
| 変数 | 値・意味 |
|---|---|
| `stfw_run_status` | 当該階層の確定ステータス（`Success` / `Warn` / `Error`）。run / scenario / bizdate / process の teardown で注入。Warn あり・Error なしの階層では `Warn`（SPEC-023-05） |
| `stfw_process_retcode` | process teardown のみ。`0`（Success）/ `3`（Warn）/ `6`（Error） |
...
|---|---|---|
| 0 | SUCCESS | Success |
| 3 | WARN | **Warn として記録して続行**（後続のステップ・プロセス・階層を止めない。SPEC-023-01） |
| 6 | ERROR | Error（後続のステップ・兄弟ノードを実行せず停止。後続ステップは Blocked として記録） |

**実行意味論（v1.2.0 で追加・REQ-023）**

- ステップ / プロセスの exit 3 は `Warn` として実行ジャーナルに記録し、後続の実行は止めない。exit 6 等（非 0・非 3）は従来どおり停止 + Blocked 伝播。
- 上位階層（process / bizdate / scenario / run）の集約は **Error > Warn > Success** の優先度（SPEC-023-02）。子に Warn があれば親も Warn（子に Error があれば親は Error）。
- `stfw run` の終了コードは全 Success=0 / Warn あり（Error なし）=3 / Error あり=6（SPEC-023-03。CI で「差分あり」を終了コードで検知できる）。
- 階層フック（§4.4）は対象外: フックの成否判定は従来どおり「0 か否か」で、非 0 は失敗（当該階層 Error）。
- **互換性**: RC 契約（0/3/6）自体は不変。v1.1 以前は成否判定が「0 か否か」で 3 も Error 扱いだったため、3 を返していたプラグインは「止まらなくなる」挙動変化がある（リリースノートで明示）。
...
| invokeWeb / invokeRest | `evidence/summary.json`（k6 end-of-test サマリ）+ `evidence/report.html`（k6 web dashboard の HTML レポート） | gitignore |
...
表中の `{process}` は **run ワークスペース内のプロセスディレクトリ**（`.stfw/runs/{run_id}/workspace/{scenario_name}/{bizdate_dir}/{process_dir}`）を指す（§5.7）。実行出力（`evidence/` / `actual/` / `result/`）は run ワークスペースにのみ生成され、`scenario/` 配下の正本には書き込まれない。この 3 つはプロセスディレクトリ直下の予約出力ディレクトリ名であり、ワークスペース複製時にも正本側のプロセスディレクトリ直下の同名ディレクトリ（旧バージョンの実行残骸）はコピーしない（§5.7）。テンプレートの `.gitignore` 除外は旧バージョンで生成された残骸の誤コミット防止のため維持する。
...
> 根拠: `internal/usecase/runscenario/{runscenario,process,env}.go`, `internal/gateway/{scriptexec,lookpath}.go`, `internal/repository/{hooks,plugin,pluginmeta,evidence,config}.go`, `internal/domain/evidence/evidence.go`, `assets/plugins/process/scripts/`（同梱 scripts プラグイン）, `assets/template/.gitignore`, `test/acceptance/testdata/script/{run_実行する場合_公開envが契約どおりであること,run_全ステップ成功の場合_Successになること,run_途中でexit6になる場合_後続がBlockedになること,run_途中でexit3になる場合_Warnで続行すること,run_Warn後にexit6になる場合_Errorが優先されること,run_dryrunの場合_executeとpostexecuteをスキップすること,run_scripts以外の独自プラグインの場合_exec契約どおり実行すること,plugin_契約を検証する場合_requires不足と接続情報直書きを検出すること}.txtar`
...
> 根拠: `assets/plugins/process/{export,import,clear}{Mysql,Postgres}/`, `internal/repository/mysqlcsv.go`（`MySQLBatchTSVToCSV`）, `internal/presentation/logger/masker.go`（`secretRegistry` / `Wrap`）, `internal/usecase/secret/secret.go`（`RegisterAll`）, `internal/presentation/cli/{run,plugin}.go`, `test/acceptance/testdata/script/{mysql_exportimportclearの場合_CSVを往復できること,postgres_exportimportclearの場合_CSVを往復できること}.txtar`
...
> 根拠: `assets/plugins/process/{export,import,clear}Redis/`, `internal/repository/rediscsv.go`（`RedisEncodeRow` / `RedisDecode` / `redisQuote`）, `internal/presentation/cli/plugin.go`（`redis-encode-row` / `redis-decode`）, `test/acceptance/testdata/script/redis_exportimportclearの場合_データを往復できること.txtar`

### 4.11 組込み compare プラグイン（Assert・外部 OSS compare-files）

Assert フェーズの組込みプラグイン。期待値（`expect/`）と収集エビデンス（`actual/`）を外部 OSS [`scenario-test-framework/compare-files`](https://github.com/scenario-test-framework/compare-files)（Go 単一バイナリ `compare_files`）で突合する。比較不一致の扱いは設定キー `on_mismatch` による選択制（SPEC-014-02 改訂）: `error`（既定）は該当ステップの Error として既存のエラー時停止・Blocked 伝播に載せ（回帰テスト運用）、`warn` は Warn として記録して後続を継続する（差分確認運用。§4.6）。
...
3. **プロジェクト共通レイアウトの注入**: `{proj}/config/plugins/process/compare/` が存在する場合、`COMPAREFILES_CLASSPATH` の末尾へ連結（既存値があれば `:` 区切り）してから起動する。
4. **起動設定のプロセスローカル優先**: プロセスディレクトリの `config/compare_files.{json,yaml,yml}`（この優先順で探索）が存在する場合、`-config` で明示指定して起動する（compare-files の既定解決は前勝ちのため、明示しないと手順 3 で注入したプロジェクト共通側が勝ってしまう）。`-config` 指定時も、欠落キーは既定解決（プロジェクト共通 → 同梱デフォルト）の値で補完される。
5. `compare_files [-config {process-local}] -od result expect actual` を実行する。

**設定・レイアウトの解決順（シナリオ横断の共通化）**

- **比較レイアウト（`compare_layout/`）**: compare-files は「同梱デフォルト → `COMPAREFILES_CLASSPATH` の各ディレクトリ → カレント → `./config`」の**後勝ちマージ**で読む。シナリオを跨いで同じレイアウト（例: エクスポート CSV の連番列を `criteria: "Ignore"`・キー列を `compareKey` にする定義）は **`{proj}/config/plugins/process/compare/compare_layout/*.json`** に置き、プロセス固有の上書きは従来どおりプロセスディレクトリの `config/compare_layout/` に置く（同一 `fileRegexPattern` は後勝ちで上書き）。
- **起動設定（`compare_files.{json,yaml,yml}`）**: 優先順は**プロセスローカル `config/`（手順 4 の `-config` で最優先） → プロジェクト共通 `{proj}/config/plugins/process/compare/`（CLASSPATH 前勝ち） → バイナリ同梱デフォルト**。プロジェクト共通に置いた起動設定は「プロジェクト既定」として機能し、プロセスローカルがあればそちらが優先される。

**設定**（config.yml の `stfw.process.compare`。上書きチェーンは §8.1）

| キー | 説明 |
|---|---|
| `compare_files_version` | 取得する compare-files のリリースタグ（既定 `v2.2.0`） |
| `on_mismatch` | 比較不一致（compare-files exit 3）の扱い。`error`（既定）= exit 6 に変換してステップ失敗（従来挙動・回帰テスト運用）/ `warn` = exit 3 のまま返して Warn 記録・続行（差分確認運用）。`error` / `warn` 以外の値は設定不正として exit 6 |

**終了コードの対応付け**

- compare-files: `0=一致（OK/Ignore）` / `3=差分（NG/LeftOnly/RightOnly）` / `6=エラー`。
- 一致（exit 0）は on_mismatch の設定によらず Success。比較エラー（exit 6 等の非 0・非 3）は常に exit 6（Error）。
- 差分（exit 3）は `on_mismatch: error`（既定）のとき **exit 6 に変換**（Error。ランナーが後続ステップを Blocked にする）、`on_mismatch: warn` のとき **exit 3 のまま返す**（ランナーが Warn として記録し後続を継続する。§4.6）。
...
5. end-of-test サマリを `${stfw_process_dir}/evidence/summary.json` へ出力する。HTML レポート `${stfw_process_dir}/evidence/report.html` は **(1)** k6 web dashboard（`--out web-dashboard=export=…`。負荷テスト等で十分なデータがある場合に k6 が生成）を優先し、**(2)** 生成されなかった場合（単発 Act 等でデータ不足）はサマリから自己完結 HTML をプラグインがフォールバック生成する（k6 が出したレポートは上書きしない）。実行前に前回の report.html を削除し、k6 失敗時もサマリからフォールバックするため report.html は常に残る。
...
- sshExec / scpPut ともエラーは一律 exit 6（Error）に正規化する。exit 6 のためランナーでは常に Error となり、後続ステップは Blocked になる（exit 3 = Warn を返す経路はない。§4.6）。

> 根拠: `assets/plugins/process/{sshExec,scpPut}/`, `test/acceptance/testdata/script/{sshExec_各ホストへ送信する場合_ファイル名昇順で実行すること,scpPut_各ホストへ配置する場合_tmp経由でdestへmvすること}.txtar`, `internal/repository/plugin.go`（shebang 付きスクリプトの 0755 展開）

### 4.14 組込み parallel プロセス（子プロセス並走・Go ネイティブ実行）

`parallel` タイプは、プロセスディレクトリ配下に定義した**子プロセス**を並走させる組込みネイティブタイプ
（scripts §4.3 と同じく、同梱プラグイン解決時は外部スクリプトを呼ばず Go ネイティブで実行する）。

**ディレクトリ規約**

```
_{seq}_{group}_parallel/
├── config/config.yml            # parallel 自体の設定（max_parallel）。必須（他プロセスと同じ）
├── _{seq}_{group}_{type}/       # 子プロセス（親プロセスと完全に同形式）
│   ├── config/config.yml        # 子の設定（必須）
│   ├── data/ …                  # 子のプラグインが要求する入力（通常プロセスと同じ）
│   └── evidence/ …              # 子のプラグインの出力（stfw_process_dir が子を指すため自然に分離）
└── _{seq}_{group}_{type}/
```

| 規則 | 内容 |
|---|---|
| 子の走査 | 走査規則（§3.4）と同一: `_` 始まりのディレクトリのみ・名前昇順。seq は**実行キューへの投入順と表示順**にのみ使う（完了順は不定） |
| 子の命名 | `_{seq}_{group}_{type}`（§3.3 のプロセスと同じパース・値オブジェクト検証） |
| 入れ子禁止 | 子のタイプに `parallel` は指定できない（validate エラー） |
| 子 0 件 | validate エラー（子を持たない parallel は定義不正） |
| validate | 子にも通常プロセスと同じ検証を適用する: ディレクトリ名形式 / プラグイン解決可否 / `config/config.yml` 存在 / 接続情報直書き禁止（§4.7）/ `plugin.yml` requires（§4.7） |

**実行セマンティクス**

1. **計画列挙**: 子ディレクトリ名を昇順で列挙し `steps_enumerated` イベントで全件 Pending 登録する
   （**子プロセス 1 件 = ジャーナル上の 1 ステップ**。dry-run でも行う）
2. **setup フック**（process 階層）を親 parallel プロセスで 1 回実行する（失敗時は teardown なしで Error）。
   **子では process 階層フックを実行しない**
3. **子の並走実行**: 各子を goroutine で実行する。実行キューへの投入は seq 昇順で、`max_parallel`
   超過分は空きが出た順に seq 昇順で開始される。**同時に起動される子同士の実開始タイミングの
   前後は保証しない**（並走のため本質的に不定）。
   各子の実行は通常プロセスと同じ内容（プラグイン解決 → config チェーン注入 → exec 契約 §4.2 の
   pre_execute → execute → post_execute。dry-run 時は pre_execute のみ）。
   組込み scripts タイプの子は Go ネイティブで実行するが、ステップイベントは記録しない
   （粒度は「子 1 件 = 1 ステップ」。子内スクリプトの逐次・Blocked 規則は §4.3 と同じ挙動でログにのみ現れる）
4. **子の記録**: 子の完了ごとに `step_end`（ステップ名 = 子ディレクトリ名、status = Success/Warn/Error、
   exit_code = 0/3/6、start_ts / end_ts = 実実行時間）を記録する。
   **子同士に Blocked は無い**（並走に順序依存が無いため、先行子が Error でも全子を必ず最後まで実行する）
5. **集約**: 親 parallel プロセスのステータス = 全子の悪い方（Error > Warn > Success。§4.6 と同じ優先度）。
   親が Error の場合は既存規則どおり後続の兄弟プロセスを実行せず停止する
6. **teardown フック**（process 階層）を親で 1 回実行する（`stfw_run_status` / `stfw_process_retcode` は親の集約結果）

**max_parallel（同時実行数の上限）**

- 実効値は env `stfw_process_parallel_max_parallel`（設定チェーンで解決。後勝ち）:
  同梱デフォルト stfw.yml（`stfw.process.parallel.max_parallel: 0`）→ プロジェクト stfw.yml →
  プロジェクト `config/plugins/process/parallel/config.yml` → プロセス `config/config.yml`
- `0` = 上限なし（全子を同時起動）。正整数 = 同時実行数の上限（超過分は seq 順に待機）。
  負数・非整数は設定不正として親プロセスを Error にする（子の計画列挙より前に検出する）
- プロジェクト全体の既定は stfw.yml、特定の parallel プロセスのみ制限したい場合はプロセスの
  `config/config.yml` で上書きする

**env / エビデンス**

- 子の env は親 parallel プロセスの env（親の config チェーン注入済み）を複製し、
  `stfw_process_dir` / `stfw_process_dirname` / `stfw_process_seq` / `stfw_process_group` /
  `stfw_process_type` / `stfw_plugin_cache_dir` を**子の値で上書き**したもの
- `stfw_process_dir` が子ディレクトリを指すため、エビデンス規約（§4.7 `{process}/evidence/` 等)は
  無変更で子ごとに分離される
- 子のプラグイン設定は通常の上書きチェーン（§8.1: プラグイン config.yml → プロジェクト
  `config/plugins/process/{子type}/config.yml` → 子の `config/config.yml`）で解決する。
  親 parallel の `config/config.yml` は parallel 自体の設定（max_parallel）のためのもの

**制約・注意**

- 子の stdout / stderr は並走のため**行単位で交錯し得る**（各ストリームの行内は壊れない）。
  行内保証のため、改行で終わらない子の最終出力には**改行を補って**転送する
- ジャーナルの `step_end` は子の完了時に記録される。**記録順はイベント直列化（排他）の獲得順**であり、
  厳密な完了時刻順は保証しない（実時刻は各イベントの `start_ts` / `end_ts` が保持する）。
  ステップはステップ名でキーされるため status / report の表示順は列挙順（seq 昇順）で安定する
  （§5 のスキーマ・状態遷移は一切変更しない）
- 部分実行 `--from` / `--only`（§3.4）で指定できるのは親 parallel プロセスまで（子は指定不可）
- `stfw new process` は子プロセスの scaffold に対応しない（子は手動作成または `scenario scaffold` §12 で生成する）
- プロジェクトに `plugins/process/parallel/` を置いた場合はプロジェクト側が優先解決され、
  exec 契約（§4.2）で実行される（ネイティブ実行は同梱解決時のみ。scripts §4.3 と同じ規則）。
  その場合も本節の子ディレクトリ規約の validate は適用される（タイプ名 `parallel` は子構造を前提とする）

> 根拠: `internal/usecase/runscenario/parallel.go`（`runParallelProcess` / `execChildren` / `execChildProcess`）, `internal/domain/scenario/tree.go`（子ノード構築・validate）, `internal/domain/run/status.go`（`WorstStatus`）, `assets/plugins/process/parallel/`, `test/acceptance/testdata/script/{run_parallelの場合_子プロセスを並走させること,run_parallelでWarnとErrorが混在する場合_悪い方を採用すること,run_maxparallelを指定する場合_同時実行数を制限すること,run_parallelのscripts子の場合_フックは親のみでステップイベントを記録しないこと,validate_parallelの子が不正な場合_エラーになること,roundtrip_parallelを含むspecを往復する場合_同一に戻ること}.txtar`
...
| event | 追加フィールド | 内容 |
|---|---|---|
| `node_start` | `parent_id`, `node_type`（`run\|scenario\|bizdate\|process`）, `attrs` | 階層の実行開始（Started）。attrs は階層別: run = `run_id` / `run_mode`（`--run` / `--dry-run`）/ `params`（指定シナリオ名の空白連結）/ 部分実行時のみ `from` / `only`（§3.4）/ resume 時のみ `resume_from`（引き継ぎ元 run_id。§5.8）、scenario = `name`、bizdate = `dirname` / `seq` / `bizdate`、process = `dirname` / `seq` / `group` / `process_type` |
| `steps_enumerated` | `steps`（string 配列・実行順） | プロセスのステップ全件を Pending 登録（scripts のスクリプト名 §4.3 / parallel の子ディレクトリ名 §4.14）。Started な process ノードに 1 度だけ記録できる。空リスト・重複ステップ名は不正 |
| `step_end`（Success / Warn / Error） | `step`, `status`, `exit_code`（int）, `start_ts`, `end_ts` | ステップの実行終了。start_ts / end_ts は実実行時間 |
| `step_end`（Blocked） | `step`, `status`（=`Blocked`） | 先行エラーによる未実行。exit_code / start_ts / end_ts を持たない |
| `node_end` | `status`（`Success` / `Warn` / `Error`） | 階層の実行終了 |

### 5.4 状態遷移

| 状態機械 | 遷移 |
|---|---|
| NodeStatus（階層: run / scenario / bizdate / process） | `Started → Success \| Warn \| Error` のみ。終了状態からの再遷移は不正 |
| StepStatus（scripts のスクリプト / parallel の子プロセス） | `Pending → Success \| Warn \| Error \| Blocked` のみ（parallel の子は Blocked にならない。§4.14） |

`Warn` は v1.2.0 で追加（REQ-023）。追加的変更のため、旧バージョンの run のジャーナル（Warn なし）もそのままリプレイでき、新旧ジャーナルが混在しても status / report / ハウスキープは壊れない（SPEC-023-04）。
...
> 根拠: `internal/domain/run/{event,run,steps,status,runid,nodeid}.go`, `internal/repository/journal.go`, `internal/usecase/status/status.go`, `test/acceptance/testdata/script/{run_全ステップ成功の場合_Successになること,run_途中でexit6になる場合_後続がBlockedになること,run_途中でexit3になる場合_Warnで続行すること,run_Warn後にexit6になる場合_Errorが優先されること,run_dryrunの場合_executeとpostexecuteをスキップすること}.txtar` の `journal_golden.jsonl`
...
| 削除対象 | `.stfw/runs/{run_id}/`（ディレクトリごと。実行ワークスペース = **エビデンスを含む**、§5.7）と `.stfw/reports/runs/{run_id}.html`（レポート不在は許容） |
...
> 根拠: `internal/repository/housekeep.go`, `internal/usecase/runscenario/housekeep.go`, `internal/domain/run/runid.go`（`Time()`）, `test/acceptance/testdata/script/run_開始時ハウスキープの場合_保存期間超過を削除すること.txtar`

### 5.7 実行ワークスペース（run workspace・同一シナリオ並走）

実行時生成物を run_id で名前空間化し、同一シナリオを含む複数の `stfw run` を同一プロジェクトで並走可能にする。

| 項目 | 仕様 |
|---|---|
| 配置 | `.stfw/runs/{run_id}/workspace/{scenario_name}/`（run ディレクトリの一部） |
| 複製契機 | run_id 採番・ジャーナル作成の直後、実行開始（run の node_start）前に、実行対象シナリオごとに `scenario/{name}/` を丸ごとコピー。コピー（resume 時はマージ含む）の失敗は実行を開始せずエラー終了（exit 6）し、空ジャーナルの run ディレクトリを削除する（best-effort・stderr へログ）。`run_id:` は複製**前**に出力するため（大きなシナリオの複製中に無出力にしない意図的な順序）、この場合は出力済みの run_id が無効になる |
| コピー規則 | 通常ファイルとディレクトリを複製し、**パーミッションを保持**する（0600 の入力を group/other へ広げない。setuid/setgid/sticky は保持しない）。通常ファイル・ディレクトリ以外（FIFO 等）は複製時エラー。シンボリックリンクは**リンク先の実体を複製**する（リンクのまま複製すると実行が正本・外部ツリーへ書き込む/相対リンクが切れるため。参照切れ・リンク循環は複製時エラー）。予約出力ディレクトリ名 `evidence/` / `actual/` / `result/` は旧バージョンの実行残骸のため、**ディレクトリ規約上のプロセス位置**（bizdate 直下のプロセスディレクトリ、および parallel の子プロセスディレクトリ）**の直下に限り**コピーしない（§4.7。`data/` 等の入力ディレクトリ配下の同名はコピーする） |
| 実行 | プロセスは複製ツリー上で実行する。階層フック（§4.4）の実体と作業ディレクトリはプロジェクト側 `plugins/{level}/_common/{phase}/` のままで、env（`stfw_scenario_dir` / `stfw_bizdate_dir` / `stfw_process_dir`）が複製側を指す（§4.5）。`scenario/` 配下の正本は読み取りのみで、実行によって変更されない |
| 同梱プラグイン展開先 | run 単位の `.stfw/runs/{run_id}/plugins/process/{type}/` へ展開する（通常プロセスは実行のたびにワイプ + 再展開。parallel の子は並走中の展開衝突を避けるため並走開始前にタイプ単位で 1 回のみ展開する（§4.14）。並走 run 間では展開が衝突しない）。`stfw plugin install` の展開先 `.stfw/plugins/` は従来どおり |
| stdout | run 開始時に `run_id: {run_id}` に続けて `workspace: {絶対パス}` を出力する（エビデンスの所在案内） |
| ライフサイクル | ワークスペースは run ディレクトリの一部としてハウスキープ（§5.6）で削除される。**エビデンス（`evidence/` / `actual/` / `result/`）も run と共に削除される**ため、保存が必要なエビデンスは `retention_days` 内に外部へ回収する。期限判定は run_id 採番時刻ベース（§5.6）のため、`retention_days` は想定される最長の run 実行時間より十分長く設定する（実行中 run のワークスペースが並走 run のハウスキープで削除されることを防ぐ） |
| 並走時に共有のままの領域 | `.stfw/reports/index.html`（全 run 共有。原子的書き込みの後勝ちスナップショットで、並走 run の終了タイミングによっては**完了済み run が Started 表示のまま残り得る**。次の `stfw run` / `stfw report` の再生成で整合する）、`.stfw/cache/plugins/`（provisioning 済み資産の読み取り共有）、プロジェクトの `plugins/` / `config/`（読み取り共有） |

> 根拠: `internal/repository/workspace.go`, `internal/repository/plugin.go`（`MaterializePlugin`）, `internal/usecase/runscenario/{runscenario,process,parallel}.go`, `test/acceptance/testdata/script/{run_実行する場合_ワークスペースへ複製して実行すること,run_実行する場合_公開envが契約どおりであること}.txtar`

### 5.8 resume（`--resume`・前 run ワークスペースの引き継ぎ）

失敗した run のやり直し（`--from` / `--only` との併用が主用途）で、前 run が生成したエビデンス・実行時生成の入力を新しい run へ引き継ぐ。

| 項目 | 仕様 |
|---|---|
| 指定 | `--resume`（値省略 = 最新の**完了済み** run）/ `--resume=<run_id>`（明示指定。`=` 区切りのみ）。`--resume=`（空値）はエラー。「最新の完了済み run」は成否を問わない（Error 完走も対象。実行中・初期化中の run はスキップ）ため、引き継ぎたい run の後に別の run（resume なしの部分実行等）を挟んだ場合は `--resume=<run_id>` で明示指定する |
| 引き継ぎ元の解決 | ハウスキープ（§5.6）後・新 run_id 採番前に解決・検証する（引き継ぎ元の不正・対象シナリオ不在は採番前に fail-fast する。マージ実行時の I/O 失敗は §5.7 複製契機と同じ扱い = run ディレクトリ削除 + 出力済み run_id は無効）。**完了済み（run の node_end 記録済み）の run のみ**引き継ぎ元にできる（実行中 run の書き込み途中ワークスペースを非原子的に取り込むことを防ぐ）。完了済み run が無い / 指定 run_id が不正・不存在・未完了 / 引き継ぎ元ワークスペースに対象シナリオが無い場合は実行を開始せずエラー（exit 6） |
| マージ規則 | 正本の複製（§5.7）を行った**後**、引き継ぎ元 run のワークスペースの同一シナリオツリーを**既存ファイルを上書きせずに**マージ取り込みする。**正本（今回の複製）が優先** = 正本への修正が resume 後の実行に反映される。同一パスの種別（ファイル/ディレクトリ）が正本と前 run で異なる場合も正本を優先し、前 run 側は取り込まない。前 run にのみ存在するもの（`evidence/` / `actual/` / `result/`、実行時生成された `data/` 等）が引き継がれる。正本から削除したファイルが前 run のワークスペースに残っている場合は引き継がれる（削除は反映されない） |
| ジャーナル | 新しい run_id の別実行として記録する（§5.1 の不変性は維持）。run の node_start attrs に `resume_from`（引き継ぎ元 run_id）を記録する（§5.3） |
| stdout | `workspace:` 行に続けて `resume_from: {run_id}` を出力する |
| 適用範囲 | 実行対象の全シナリオに適用する（`--from` / `--only` なしでも指定可。ただし収集プロセスが再実行されれば引き継いだエビデンスは上書きされる） |

> 根拠: `internal/repository/workspace.go`（`MergeRunWorkspace`）, `internal/usecase/runscenario/runscenario.go`, `internal/presentation/cli/run.go`, `test/acceptance/testdata/script/run_resumeを指定する場合_前runの生成物を引き継ぐこと.txtar`
...
|---|---|---|---|
| run | `stfw run` | `stfw.run_id`, `stfw.node.type`=`run`, `stfw.node.id`, `stfw.run.mode`（`run` / `dry-run` — `--` 接頭辞を除去して正規化） | Success → Ok / Warn → **Ok + 属性 `stfw.node.status`=`Warn`** / Error → Error |
...
| step（Success / Warn / Error） | スクリプト名 | `stfw.run_id`, `stfw.node.type`=`step`, `stfw.node.id`（`{process_node_id}+{step名}`）, `stfw.step.status`, `stfw.step.exit_code`（int64） | Success / Warn → Ok / Error → Error（メッセージ: `step {名} failed with exit_code {n}`） |
...
OTel のスパンステータスには Warn 相当がない（Ok / Error / Unset のみ）ため、Warn はスパンステータス Ok + 属性で表現する（SPEC-024-03）: 階層ノードは `stfw.node.status`=`Warn`（Warn のときのみ付与）、step は既存の `stfw.step.status` が `Warn` を持つ。
...
> 根拠: `internal/usecase/runscenario/otel.go`, `internal/domain/notify/{projector,span}.go`, `internal/gateway/otlptrace.go`, `internal/usecase/runscenario/otel_test.go`, `internal/domain/notify/projector_test.go`, `test/acceptance/testdata/script/run_OTLPを設定する場合_トレースを送信すること.txtar`
...
- ステータスバッジの配色: Success=緑 / **Warn=黄** / Error=赤 / Blocked=橙灰。Warn の一覧性により、差分確認モード（compare `on_mismatch: warn`）ではレポートがそのまま「比較 NG の鳥瞰」ビューになる（SPEC-024-02）
...
> 根拠: `internal/usecase/runscenario/reporter.go`, `internal/repository/report.go`, `internal/gateway/htmlwriter.go`, `assets/report/{index,run}.html.tmpl`, `internal/domain/run/duration.go`, `test/acceptance/testdata/script/report_run中とオンデマンドの場合_HTMLを生成すること.txtar`
...
| `stfw.process.parallel.max_parallel` | `0`（上限なし） | ○（組込み parallel プロセスの同時実行数の上限。§4.14） |
...
> 根拠: `internal/repository/config.go`, `assets/config/stfw.yml`, `assets/template/stfw.yml`, `internal/repository/plugin.go`（ProcessConfigEnv）, `internal/usecase/runscenario/env.go`（exportConfigEnv）, `test/acceptance/testdata/script/{run_実行する場合_公開envが契約どおりであること,run_scripts以外の独自プラグインの場合_exec契約どおり実行すること,run_configの変数展開の場合_stfwymlの値を参照できること}.txtar`
...
> 根拠: `internal/domain/project/{secret,inventory}.go`, `internal/repository/{secret,inventory}.go`, `internal/usecase/{secret/secret.go,sshtrust/sshtrust.go}`, `internal/gateway/sshkeyscan.go`, `internal/presentation/logger/masker.go`, `test/acceptance/testdata/script/{secret_鍵生成とパスワード設定の場合_暗号化して保存すること,secret_旧形式の場合_ageへ移行できること,sshTrust_未登録ホストの場合_knownhostsへ登録すること,inventory_グループを問い合わせる場合_ホストを解決すること}.txtar`
...
- **parallel の子プロセス（§4.14）**: `type: parallel` のプロセスエントリは、任意で `processes:`
  （ProcessSpec と同スキーマの子プロセスリスト）を持てる。`reverse` は子を含めて出力し、
  `scaffold` は子ディレクトリ（`metadata.yml` + `config/config.yml`）も生成・同期（`--sync` の
  追加/維持/削除）する。parallel 以外のプロセスに `processes:` を書くとエラー。子への入れ子
  （子の `processes:`）もエラー。
...
> `test/acceptance/testdata/script/{reverse_treeを入力する場合_specとdocを生成すること,scaffold_specを入力する場合_treeを生成すること,roundtrip_specを往復する場合_同一に戻ること}.txtar`
