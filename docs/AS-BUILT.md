...
| 項目 | 仕様 |
|---|---|
| バージョン | `stfw --version`。ビルド時に `-ldflags -X ...cli.Version=` で注入（未注入時 `1.1.1-dev`） |
...
> 根拠: `assets/plugins/process/compare/`, `test/acceptance/testdata/script/{compare_expectとactualを突合する場合_合否を判定すること,compare_プロジェクト共通レイアウトがある場合_探索パスへ注入されること}.txtar`, compare-files `internal/cli/runner.go`（`configSearchDirs`/`LoadLayoutManager`）・`internal/bulk/dir.go`（`relPathList`/`Counts.ProcessStatus`）・`internal/status/status.go`（`ExitCode`）
...
> 根拠: `internal/domain/run/{event,run,steps,status,runid,nodeid}.go`, `internal/repository/journal.go`, `internal/usecase/status/status.go`, `test/acceptance/testdata/script/{run_全ステップ成功の場合_Successになること,run_途中でexit6になる場合_後続がBlockedになること,run_dryrunの場合_executeとpostexecuteをスキップすること}.txtar` の `journal_golden.jsonl`
