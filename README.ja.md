...
- **順序保証・エラー時停止**: ファイル名昇順で逐次実行し、エラー以降のステップは `Blocked` として記録。`Warn` (exit 3) は記録して続行するため、狙い通りの差分を全シナリオで鳥瞰する運用もできる (`stfw run` は Warn ありのとき exit 3)
...
- **実行分離・並走**: run ごとの実行ワークスペース (`.stfw/runs/{run_id}/workspace/`) にシナリオを複製して実行。`scenario/` 正本には書き込まず、同一シナリオを含む複数 run を並走できる。`--resume` で前 run の生成物を引き継いで途中から再開できる
- **ハウスキープ**: `stfw run` の開始時に保存期間 (`stfw.housekeep.retention_days`) を過ぎた実行結果 (エビデンス含む) を自動削除

> 📋 **テスト管理者・PM の方へ** — 手順書と人海戦術からの卒業を検討している方は、[管理者向けガイド](docs/FOR-MANAGERS.md) からお読みください。
...
## AI エージェントでシナリオを作る (Claude Code)

SUT の設計情報 (要件定義・OpenAPI Spec・DDL 等) とテストのゴールから、シナリオ一式
(inventory / secret セットアップスクリプト / カスタムプラグイン / scenario tree / 期待値) を
`stfw validate` 通過まで生成する [Claude Code](https://claude.com/claude-code) 用エージェントスキル
[`stfw-scenario`](.claude/skills/stfw-scenario/) を同梱しています。

```console
$ npx skills add scenario-test-framework/stfw --skill stfw-scenario -a claude-code
```

インストール後、stfw プロジェクトで Claude Code に「この OpenAPI と DDL からシナリオを作って」
のように依頼すると、参照ドキュメントの収集・設計確認から validate 通過までを対話的に進めます。
...
- 出力: リターンコード（`0` = Success / `3` = Warn: 記録して続行 / `6` = Error: 停止して後続を Blocked）
...
| 任意 | `parallel` | プロセスディレクトリ配下に定義した子プロセスを並走 (親のステータスは全子の悪い方。Go ネイティブ) |
...
- [examples/daily-balance](examples/daily-balance/) — 業務日付をまたぐ日次残高バッチの実行可能サンプル。postgres + トイ REST API を同梱し `./run.sh` で end-to-end 実行できます（Arrange→Act→Collect→Assert を組込みプラグイン + カスタムプラグイン実装例 2 つで構成）
...
| `stfw run [--dry-run] [--from <path>] [--only <path>] <scenario...>` | シナリオの一括自動実行。`--from` は指定ノードから再開、`--only` は指定サブツリーのみピンポイント実行 (path は `{bizdate_dir}[/{process_dir}]`) |
