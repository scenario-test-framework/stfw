...
```
config/plugins/process/importMasterData/data/{database}/{table}.csv
```
...
```
config/plugins/process/importMasterData/data/
└── appdb/
...
| `tables` | 取り込む共通データ（テーブル名）のリスト。`config/plugins/process/importMasterData/data/{database}/{table}.csv` を解決する |
