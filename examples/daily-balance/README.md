...
- [実行](#実行)
  - [実行状況の可視化](#実行状況の可視化)
- [シナリオの流れ](#シナリオの流れ)
- [カスタムプラグイン（importMasterData / updateBizdate）](#カスタムプラグインimportmasterdata--updatebizdate)
  - [importMasterData — 共通マスタデータの投入](#importmasterdata--共通マスタデータの投入)
  - [updateBizdate — SUT の業務日付を進める](#updatebizdate--sut-の業務日付を進める)
- [わざと失敗させてみる](#わざと失敗させてみる)
  - [パターン 1: `error` — 最初の比較 NG で止まる（既定）](#パターン-1-error--最初の比較-ng-で止まる既定)
  - [直して部分実行で確認する（`--only` / `--from`）](#直して部分実行で確認する--only----from)
  - [パターン 2: `warn` — 完走して比較 NG を鳥瞰する](#パターン-2-warn--完走して比較-ng-を鳥瞰する)
- [ドキュメント / spec（ラウンドトリップ）](#ドキュメント--specラウンドトリップ)
- [ディレクトリ](#ディレクトリ)
- [比較レイアウトの共通化](#比較レイアウトの共通化)
- [設定の共通化（プロジェクト共通 config とプロセス差分）](#設定の共通化プロジェクト共通-config-とプロセス差分)
...
- **HTML レポート**（stfw 内蔵）: run の階層ごとの Success/Warn/Error を nginx で配信（`http://localhost:8088`）。
...
> エビデンス等の実行時生成物は `scenario/` 配下ではなく、run ごとの実行ワークスペース
> `stfw/.stfw/runs/{run_id}/workspace/daily-balance/` 内の同じ相対位置に出力されます
> （run 開始時の stdout `workspace:` 行にパスが表示されます）。
...
|---|---|---|---|---|
| `_010_20240101` | `_10_arrange` | clearPostgres | Arrange | users / accounts / transactions を全行削除 |
| | `_15_arrange` | **parallel**（組込み・並走） | Arrange | 互いに独立な下記 2 つの子プロセスを並走させる |
| | ├ `_10_master` | **importMasterData**（カスタム） | Arrange | 口座名義マスタ users を config 内の CSV から投入 |
| | └ `_20_balance` | importPostgres | Arrange | 初期残高 CSV を投入 |
...
`_010` の `_15_arrange` は組込み **parallel** の実装例です。マスタ投入（users）と初期残高投入
（accounts）は互いに FK 依存が無いため、`_15_arrange_parallel/` 配下の子プロセス
（`_10_master_importMasterData` / `_20_balance_importPostgres`）として並走させています。
子はジャーナル上のステップとして記録され、親のステータスは全子の悪い方に集約されます。
...
`_010` の `_15_arrange_parallel` 配下の `_10_master` は `importMasterData`
...
比較不一致の扱いは compare プラグインの `on_mismatch` 設定で **2 つの運用モード**から選べるため、
ここで両方を試します。あわせて、失敗からの立て直しに使う**部分実行**（`--only` / `--from`）も
ここで試します。

| モード | `on_mismatch` | 不一致時の挙動 | 用途 |
|---|---|---|---|
| 回帰テスト運用（既定） | `error` | compare が **Error** → 後続は**実行されない**、exit code `6` | CI で回帰を即検知して止める |
| 差分確認運用 | `warn` | compare が **Warn** → **後続を継続**、exit code `3` | 完走させて全比較結果を鳥瞰する |

### パターン 1: `error` — 最初の比較 NG で止まる（既定）
...
`_020` の compare が差分を検出して **Error** になり、後続の `_030_20240102`（Day2）は
実行されません（レポートにも現れません）。回帰を即検知できる一方、Day2 の比較結果は分かりません。

### 直して部分実行で確認する（`--only` / `--from`）

Error を直すとき、毎回シナリオ全体を回し直す必要はありません。**部分実行**を使います
（パスはシナリオ相対の `{bizdate_dir}[/{process_dir}]`。`--from` / `--only` は排他）。

```sh
# 1. 失敗した compare だけをピンポイント再実行し、NG を再現する (全体を回さないので数秒)
docker compose run --rm stfw run --only _020_20240101/_50_assert_compare daily-balance
echo $?   # => 6 (期待値はまだ間違ったまま)

# 2. 期待値を直し (9999 → 1500)、compare だけで修正を確認する
vi stfw/scenario/daily-balance/_020_20240101/_50_assert_compare/expect/_40_collect_exportPostgres/appdb/accounts.csv
docker compose run --rm stfw run --only _020_20240101/_50_assert_compare daily-balance
echo $?   # => 0 (修正 OK)

# 3. エラー箇所から最後まで再開する (Day1 のデータ投入・Act はやり直さない)
docker compose run --rm stfw run --from _020_20240101/_50_assert_compare daily-balance
echo $?   # => 0 (compare 以降 → Day2 まで完走)
```

- **`--only`**: 指定サブツリーだけ実行する。シナリオ変更時のピンポイント動作確認向け
  （「期待値や比較レイアウトを直す → compare だけ回す」の高速イテレーション）
- **`--from`**: 指定ノードから最後まで実行する。準備不足などで Error 終了した run を、
  直した箇所から再開する用途向け
- スキップしたノードの副作用（データ投入等）は再現されません。部分実行は **DB 等の外部状態が
  前回 run の続きで残っていること**が前提です（本例ではコンテナが起動している限り成立）

### パターン 2: `warn` — 完走して比較 NG を鳥瞰する

もう一度 Day1 の期待残高を 1500 → 9999 に変えたうえで、今度は比較 NG を **Warn** として
扱ってみます。

プロジェクト共通の compare 設定
[`stfw/config/plugins/process/compare/config.yml`](stfw/config/plugins/process/compare/config.yml)
の `on_mismatch` を `warn` に書き換えます（1 箇所の変更で全 compare プロセスが切り替わります）。
...
stfw:
  process:
    compare:
      ...
      on_mismatch: warn # error から書き換え
```

```sh
docker compose run --rm stfw run daily-balance
echo $?   # => 3 (Warn あり・Error なし)
```

`_020` の compare は **Warn**（黄）として記録され、**Day2 の `_030` まで完走**します。
HTML レポート（`http://localhost:8088`）では Warn バッジが黄色で一覧表示されるため、
**どの業務日のどの比較が NG だったかを 1 枚で鳥瞰**できます。期待値の洗い替えや
仕様変更直後の「差分がどこまで波及しているか」の確認に向いています。

試し終えたら、期待値と `on_mismatch` を元に戻します。

```sh
git checkout -- stfw/scenario/daily-balance/_020_20240101/_50_assert_compare/expect \
                stfw/config/plugins/process/compare/config.yml
```
...
| `stfw scenario scaffold <spec.yml> [--sync]` | ディレクトリ骨格 | spec → tree を生成／差分同期（往復の入口） |
...
├── run.sh              # 一発実行スクリプト
├── compose.yaml        # SUT(postgres + api) + jaeger(トレース) + stfw + nginx(レポート)
...
- **transactions.json**: 連番 `id`（BIGSERIAL。clear の全行 DELETE ではシーケンスが
  リセットされず再実行のたびにずれる）を `criteria: "Ignore"` で比較除外し、`account_id` + `bizdate` を
...
レイアウトの書き方は compare-files リポジトリの [比較レイアウトリファレンス](https://github.com/scenario-test-framework/compare-files/blob/master/docs/compare_layout.md) と [agent skills](https://github.com/scenario-test-framework/compare-files/tree/master/.claude/skills/compare-layout) を参照してください。
